...
def fft_calc(data_vec):
    ...
    freq_vec = (float(samp_rate)*np.arange(0, int(N_fft/2))) / \
        N_fft  # fft frequency vector
    fft_data_raw = np.abs(np.fft.fft(data_vec))  # calculate FFT
    fft_data = fft_data_raw[0:int(N_fft/2)] / \
        float(N_fft)  # FFT amplitude scaling
...
def plotter(plt_1=0, plt_2=0):
    ...
    if plt_2:
        ...
        t_spec = np.reshape(np.repeat(t_spectrogram, np.shape(
            freq_array)[1]), np.shape(freq_array))
        y_plot = fft_array  # data array
        # frequency vs. time/amplitude
...
if __name__ == "__main__":
    ...
    data_chunks, data_frames, t_0 = data_grabber(
        record_length)  # grab the data
    ...
    ###########################
    #
    _, _, _, fft_noise, _, _, _ = data_analyzer(
        noise_chunks)  # analyze recording
    t_vec, data, freq_vec, fft_data,\
        freq_array, fft_array, t_spectrogram = data_analyzer(
            data_chunks)  # analyze recording
